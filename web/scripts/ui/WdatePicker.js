...
(function() {
	var $ = {
		$langList : [ {
			name : "scripts/ui/i18n/datetime.locale-en",
			charset : "UTF-8"
		}, {
			name : "scripts/ui/i18n/datetime.locale_zh-cn",
			charset : "UTF-8"
		}, {
			name : "scripts/ui/i18n/datetime.locale_zh-tw",
			charset : "GBK"
		} ],
		$skinList : [ {
			name : "default",
		...
		getRealLang : function() {
			...
			for ( var A = 0; A < _.length; A++)
				if (_[A].name.indexOf(this.lang) > 0)
					return _[A];
			return _[0]
		}
	};
	...
	A = J();
	if ($.$wdate)
		K(A + "themes/common/my97datepicker/WdatePicker.css");
	U = X;
	if ($.$crossFrame) {
		try {
	...
	function J() {
		...
		for ( var B = 0; B < $.length; B++) {
			...
			_ = _.substr(0, _.toLowerCase().indexOf("scripts"));
			A = _.lastIndexOf("/");
			if (A > 0)
				_ = _.substring(0, A + 1);
			if (_)
	...
	function I(G, _) {
		...
		if (_ || G.eCont || new RegExp(/input|textarea|div|span|p|a/ig).test(D))
			...
			return;
		if (G.lang == "auto")
			G.lang = R ? "datetime.locale_"
					+ navigator.browserLanguage.toLowerCase()
					: "datetime.locale_" + navigator.language.toLowerCase();
		if (!G.eCont)
			for ( var C in G)
				$dp[C] = G[C];
		if (!$dp.dd
		...
		function E(F, E) {
			...
			var G = [
					...
					"</script><script src=", A, H.name, ".js charset=",
					H.charset, "></script>" ];
			for ( var I = 0; I < C.length; I++)
				if (C[I].name == E.skin)
					G.push("<link rel=\"stylesheet\" type=\"text/css\" href=\""
							+ A + "themes/common/my97datepicker/" + C[I].name
							+ "/datepicker.css\" charset=\"" + C[I].charset
							+ "\"/>");
			G
					.push("<script type=\"text/javascript\" src=\""
							+ A
							+ "/scripts/ui/calendar.js?\"+Math.random()+\" charset=\"gb2312\"></script>");
			G
