 ...
 * @version 1.0
 * 如需更多功能请联系chenlei@harmonytech.com.cn
 */
(function($){

    $.editor = {
        defaultOptions: {
            // 宽度不包括 margins, borders or padding
            width: 500,
            //高度不包括 margins, borders or padding
            height: 250,
            // 工具栏所有的控制按钮
            ...
            "Georgia,Impact,Sans Serif,Serif,Tahoma,Trebuchet MS,Verdana,宋体,新宋体,仿宋体,楷体,微软雅黑",
            //字体大小
            sizes: "1,2,3,4,5,6,7",
            // 标题样式
            styles: [["普通段落", "<p>"], ["标题1", "<h1>"], ["标题2", "<h2>"], ["标题3", "<h3>"], ["标题4", "<h4>"], ["标题5", "<h5>"], ["标题6", "<h6>"]],
        ...
        buttons: {
            ...
            "removeformat,清除格式,|" +
            "bullets,符号列表,insertunorderedlist|" +
    ...
    $.fn.editor = function(options){
        ...
        //循环所有TEXTAREA创建editor
        this.each(function(idx, elem){
            if (elem.tagName == "TEXTAREA") {
                var data = $.data(elem, EDITOR);
                if (!data){
                    data = new editor(elem, options);
					if(!$(elem).hasClass("texteditor"))
						$(elem).addClass("texteditor");
				} 
		...
		$.extend($.fn, {
			val:function(v){
				if(typeof v == 'undefined' || v===null){
					return temp_val.apply(this);
				}else{
					temp_val.apply(this, [v]);
    ...
    var BACKGROUND_COLOR = "backgroundColor", BUTTON = "button", BUTTON_NAME = "buttonName", CHANGE = "change", EDITOR = "editor", CLICK = "click", DISABLED = "disabled", DIV_TAG = "<div>", TRANSPARENT = "transparent", UNSELECTABLE = "unselectable", MAIN_CLASS = "cleditorMain", TOOLBAR_CLASS = "cleditorToolbar", GROUP_CLASS = "cleditorGroup", BUTTON_CLASS = "cleditorButton", DISABLED_CLASS = "cleditorDisabled", DIVIDER_CLASS = "cleditorDivider", POPUP_CLASS = "cleditorPopup", LIST_CLASS = "cleditorList", COLOR_CLASS = "cleditorColor", PROMPT_CLASS = "cleditorPrompt", MSG_CLASS = "cleditorMsg", // Test for ie
 ...
 buttons = $.editor.buttons;
    ...
    // editor - creates a new editor for the passed in textarea element
    editor = function(area, options){
    
        var editor = this;
        
        // Get the defaults and override with options
        editor.options = options = $.extend({}, $.editor.defaultOptions, options);
        
        // Hide the textarea and associate it with this editor
        var $area = editor.$area = $(area).hide().data(EDITOR, editor).blur(function(){
    ...
    var fn = editor.prototype, // Expose the following private functions as methods on the editor object.
    // The closure compiler will rename the private functions. However, the
    // exposed method names on the editor object will remain fixed.
    ...
    function imagesPath(){
        var cssFile = "jquery.editor.css", href = $("link[href$='" + cssFile + "']").attr("href");
    ...
    function refreshButtons(editor){
        ...
        $.each(editor.$toolbar.find("." + BUTTON_CLASS), function(idx, elem){
            ...
            var $elem = $(elem), button = $.editor.buttons[$.data(elem, BUTTON_NAME)], command = button.command, enabled = true;
